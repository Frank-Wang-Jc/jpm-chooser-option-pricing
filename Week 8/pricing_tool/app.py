...
from pricing_engine import METADATA, FEATURE_CONTEXT, price_contract as engine_price_contract
...
def dashboard():
    ...
    offline = os.environ.get("JPM_OFFLINE") == "1"
    if "snapshot" not in st.session_state:
        with st.spinner("Loading market inputs and rebuilding model features..."):
            st.session_state.snapshot = load_cached_snapshot() if offline else update_snapshot()
        st.session_state.last_refresh = time.monotonic()
    elif not offline and time.monotonic() - st.session_state.last_refresh >= 900:
    ...
    with st.sidebar:
        ...
        st.caption("Automatic refresh every 15 minutes while this page is open. Daily completed observations.")
        if snapshot.get("source_status"):
            st.json(snapshot["source_status"], expanded=False)
        st.divider()
    ...
    def price_contract(*args, **kwargs):
        return engine_price_contract(*args, feature_context=snapshot.get("feature_context", FEATURE_CONTEXT), **kwargs)
    ...
    with tab_notes:
        ...
        **Feature context.** All engineered inputs are rebuilt through {result['feature_context_date']}.
        User input changes define counterfactual scenarios around this feature snapshot. The ML rate feature uses DGS10, matching training. Online BSM pricing uses DGS1 when available.
